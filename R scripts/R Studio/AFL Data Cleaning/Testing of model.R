...
for (i in 0:(round)) {
  m <- fetch_results_afl(season = season,round_number = i)
  ss1 <- setdiff(names(m), names(matches))
  m <- m %>% select(-c(ss1))
...
Next_round_weather <- matches %>% filter(round.roundNumber == round, round.year == season) %>% select(match.utcStartTime,match.name,weather.weatherType)

matches <- matches %>% filter(round.roundNumber != round, round.year != season)
...
for (i in 0:(round-1)) {
  p <- fetch_player_stats_afl(season = season, round_number = i)
  ss1 <- setdiff(names(p), names(player_stats))
  p <- p %>% select(-c(ss1))
...
player_stats <- player_stats %>% filter(round.roundNumber != round, year(player_stats$utcStartTime) != season)
...
names(Next_round)[names(Next_round) == "weather.weatherType"] <- "Next_round_weather"
